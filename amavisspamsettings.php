 ...
 */
class amavisspamsettings extends rcube_plugin {
	...
	function init()  {
		...
		// add Tab label
		//$rcmail->output->add_label('amavisspamsettings');
		$this->register_action('plugin.amavisspamsettings',
			array($this, 'amavisspamsettingsInit'));

		$this->register_action('plugin.amavisspamsettings-save',
			array($this, 'amavisspamsettingsSave'));

		$this->include_script('amavisspamsettings.js');
		$this->include_stylesheet("amavisspamsettings.css");
	}
	 ...
	 *
	 */
	function amavisspamsettingsInit()  {
		$this->register_handler('plugin.body', array($this, 'amavisspamsettingsForm'));
		$rcmail = rcmail::get_instance();
		$rcmail->output->set_pagetitle($this->gettext('amavisspamsettingstitle'));
	 ...
	 *
	 */
	function amavisspamsettingsForm()  {
		$rcmail = rcmail::get_instance();
		...
		$quarantaineMailAddress =
			$rcmail->config->get('amavisspamsettings_ldap_quarantaine_mail_address');
		...
		$table->add(
			array('class' => 'amavisspamsettings-leftColumn'),
			html::label($field_id, Q($this->gettext('amavisSpamModifiesSubj'))
		));

		$table->add(
			array('class' => 'amavisspamsettings-rightColumn'),
		...
		for($i=0; $i < count($spamLevelNames); $i++) {
			...
			$spamLevelTable->add(
				array(
					'valign' => 'top',
					'class' => 'amavisspamsettings-spamNames'
				),
			...
			$spamLevelTable->add(
				array(
					'valign' => 'top',
					'class' => 'amavisspamsettings-spamTexts'
				),
		...
		$spamSettingsOut = "<fieldset class=\"fieldSet\"><legend>" .
			$this->gettext('amavisspamsettings') . "</legend><br />";
		...
		$buttonAddBlacklistSender = html::p(
			...
			$rcmail->output->button(
				array(
					...
					'class'   => 'amavisspamsettings-button',
		...
		$saveButton =  html::p(
			...
			$rcmail->output->button(
				array(
					'command' => 'plugin.amavisspamsettings-save',
					'type'    => 'input',
		...
		$out = html::div(
			...
			$this->gettext('amavisspamsettingstitle')
		) . "<br />";
		...
		$rcmail->output->add_gui_object('amavisspamsettingsform', 'amavisspamsettings-form');
		$rcmail->output->add_script(
			"rcube_find_object('buttonAddBlacklistSender').value = "
			. "rcmail.gettext('addButton', 'amavisspamsettings');"
		);

		// -----Generate the form tag around it and return the merged result

		return $rcmail->output->form_tag(array(
			'id'     => 'amavisspamsettings-form',
			'name'   => 'amavisspamsettings-form',
			'method' => 'post',
			'action' => './?_task=settings&_action=plugin.amavisspamsettings-save',
		), $out);
	}
	...
	private function loadUserData() {
		$returnData = array();
		$config          = rcmail::get_instance()->config;
		$ldapHost        = $config->get('amavisspamsettings_ldap_host',    'localhost');
		$baseDN          = $config->get('amavisspamsettings_ldap_basedn',  'dc=company,dc=ch');
		$adminDN         = $config->get('amavisspamsettings_ldap_adminDN', 'cn=config');
		$adminPW         = $config->get('amavisspamsettings_ldap_adminPW', 'password');
		$currentUserName = $_SESSION['username'];
		$searchAttribute = $config->get('amavisspamsettings_ldap_user_search_attribute', 'mail');
		...
		$ldapConnection = ldap_connect($ldapHost, $config->get('amavisspamsettings_ldap_port'));
	 ...
	 */
	function amavisspamsettingsSave()  {
		$rcmail = rcmail::get_instance();
		...
		$config = rcmail::get_instance()->config;
		$rcmail->output->set_pagetitle($this->gettext('amavisspamsettingstitle'));
		$quarantaineMailAddress = $rcmail->config->get('amavisspamsettings_ldap_quarantaine_mail_address');
		$hasError = false;
		...
		rcmail_overwrite_action('plugin.amavisspamsettings');
		$rcmail->output->send('plugin');
	}
	 ...
	 *			4: String: The mail address to which qurantained mails are sent. Normaly central mail account, configured in config value: "amavisspamsettings_ldap_quarantaine_mail_address"
